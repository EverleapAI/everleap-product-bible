...
The Explore Phase B two-layer content model (`scripts/2026-07-04-explore-catalog.sql`; see `063_EXPLORE_ARCHITECTURE.md`). `explore_paths` is the shared, user-independent, cacheable content catalog and is live. `explore_path_matches` is now **actively read and written** by `src/lib/explore/careerMatch.ts` (`saveMatches` / `loadWorkMatches`) and served by `src/functions/getExplorePaths.ts` for the Work lane (`personalized: true`) — matching for Work is done server-side, not client-side. (The earlier "scaffolding, not yet used by any code" note no longer applies.)
...
### `onet_occupations`
O*NET occupation-profile cache — one row per occupation, read/written by `src/lib/explore/onetCache.ts` (migration `006_onet_occupations.sql`). Rows are served from cache until stale, then refetched (45-day TTL on `fetched_at`).
- `code` (pk, canonical O*NET-SOC code e.g. `"13-1151.00"`), `soc`, `title`, `profile` (jsonb — the cached O*NET occupation profile), `fetched_at`.

### `user_interest_profiles`
Per-user RIASEC (Holland Code) interest profile, read/written by `src/lib/explore/interestProfile.ts` (migration `007_user_interest_profiles.sql`). Feeds the interest → O*NET crosswalk in the Work career-matching path.
- `user_id` (pk, fk → `users`), six 0-40 dimension scores (`realistic`, `investigative`, `artistic`, `social`, `enterprising`, `conventional`), `source` (`'inferred'` | `'assessment'`).
...
1. **No automated migration runner.** Numbered SQL migrations now exist at `apps/everleap-api/migrations/001..007` alongside the dated `scripts/*.sql` files, so newer schema changes (O*NET cache, interest profiles) are captured in versioned files — but they are still applied by hand, with no runner that tracks and applies them automatically, and older tables that predate this convention still exist only in the live database. Any environment rebuild (new dev DB, disaster recovery) therefore still depends partly on tribal knowledge, not a single reproducible command. The duplicate constraints and the manually-seeded-but-ungrepped `question_concept_mappings` data (see "Orphaned Tables" above) are both direct, observed consequences of the older no-migrations era. **Still open** — a real runner remains the next thing worth building if the schema is touched again, since it's the root cause behind most of the other items below.
