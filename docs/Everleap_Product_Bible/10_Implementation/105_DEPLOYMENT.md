...
`AzureWebJobsStorage`, `FUNCTIONS_WORKER_RUNTIME`, `POSTGRES_URL`, `RESEND_API_KEY`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_VERIFY_SERVICE_SID`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `ONET_API_KEY` (O*NET grounding/career matching), `BLS_API_KEY` (BLS wage data), `PROMPT_LAB_PASSCODE` (Prompt Lab unlock), `WEBAUTHN_RP_ID`, `WEBAUTHN_ORIGIN` (passkeys), `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY`
