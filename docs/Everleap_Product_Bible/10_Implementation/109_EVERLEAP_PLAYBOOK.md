...
There is no automated migration runner (`100_DATABASE_SCHEMA.md`) — migrations are applied by hand. Newer schema changes do live in versioned files: numbered SQL migrations at `apps/everleap-api/migrations/001..007` alongside dated `apps/everleap-api/scripts/*.sql`. If you create a table, add it to that set (a numbered migration, or a dated script following `create-story-answer-table.ts` as a template) even though some older tables predate the convention — don't let the schema's existing debt be a reason to add more of it.

## "Something in production is wrong and I need to trace it"
Generation requests are fully observable via the `user_generation_requests` table (status, attempts, error_message, timestamps) — start there for anything generation-related. For guidance content itself, `source_snapshot` on `user_page_guidance` and `user_science_insights` preserves what evidence produced a given generation. AI cost/usage is in `ai_usage_audit`, queryable via `GET /ai/usage/summary` — every live generator, including the Insights Summary call, is tracked there now (`104_PROMPT_CATALOG.md`).
...
2. **A couple of generation targets are still registered but not implemented** — only `agent:coach` and `agent:mentor` remain stubs that log and return in `generationDispatcher.ts`. (`page:explore`, `recommendation:actions`, and — most recently — `recommendation:careers` used to be stubs too but are now live; `recommendation:careers` dispatches to `generateCareerMatchesForUser`.) If a page appears to silently not update, check whether its target is actually wired up in `generationDispatcher.ts`.
...
4. **The Insights Summary card — including its Superpowers and Watchouts bullets — is produced by a single AI call** (`summaryNarrative.ts`'s `generateSummaryContent`). There is no longer a separate deterministic "signals"/dot-rating pass; `summarySignals.ts` has been removed. The one call returns headline, body, detail, three superpowers bullets, three watchouts bullets, and three tiny tasks together.
5. **Two parallel answer tables exist** (`user_question_answers` for onboarding, `story_question_answers` for Story) with no relationship between them. Don't assume querying one gives you the full picture. (The old "two sources of truth for which sciences exist" caveat no longer applies — the sciences DB table was dropped, and `SCIENCE_DEFINITIONS` in `scienceRegistry.ts` is now the sole source.)
