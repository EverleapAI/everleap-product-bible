...
2. **A few generation targets are still registered but not implemented** — `recommendation:careers`, `agent:coach`, and `agent:mentor` are stubs that log and return in `generationDispatcher.ts`. (`page:explore` and `recommendation:actions` used to be stubs too but are now live.) If a page appears to silently not update, check whether its target is actually wired up in `generationDispatcher.ts`.
